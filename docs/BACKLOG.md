...
- **Estado:** ✅ implementada (v0.19.0) · **puerta direccional + identidad BLE dedicada ❌ descartadas (v0.23.0, decisión del usuario)**; bermuda usable por las casillas de fuente existentes · **Módulos:** núcleo/zonas · **Valor:** Alta · **Esfuerzo:** L
...
- **Implementado (v0.21.0):** módulo **singleton** `MODULE_ENERGY`; motor puro `energy_engine.py` (`tariff_state` sensor+tramos fijos, `import_headroom`, `surplus` gated, `scarcity`, `resolve_context`); `coordinator_energy.py` publica `DATA_ENERGY` (patrón F32/F37) y avisa a consumidores; sensores Margen de red/Tarifa/Escasez + Excedente FV (gated); **consumidor**: el `import_headroom_w` aprieta el presupuesto de F03 en `_peak_step` (degrada a N cargas sin medidor; OFF en comunitaria).
- **Implementado (v0.24.0, §8.2):** **agregación de casa** — el coordinator suma el `energy_kwh` de cada módulo (DC/DV/DS, F06) en `house_kwh` (sensor **Consumo de casa** → panel de Energía) y, con sensor de precio, acumula `house_cost` (ΔkWh×precio, coste **bruto**, sensor **Coste de casa** restaurado); helper puro `energy_engine.add_cost`.
- **Implementado (v0.25.0, §8.3 REQ-TAR-4):** **sesgo de tarifa en DC** — DC lee `tariff_state` de `DATA_ENERGY` y el motor puro modula el lead de anticipación (`tariff_lead_mult`: barato ×1.5 preacondiciona, pico ×0.6 recorta) + sesgo de base opcional (`tariff_bias`, off por defecto); tunables en la categoría `tariff_bias`; back-compat sin módulo Energy. **Diferido:** §8.5 FV/excedente, §8.6 carga VE (⚠️ validación externa), **§8.2 balance neto** (consumo vs red import/export y FV), potencia instantánea total, DS respetando headroom.
...
| **F03** | ✅ implementada | Anti-pico opt-in: árbitro de casa `peak.py` (`PeakLoadHub`), canales separados clima/persianas; límite por N cargas o kW + escalonado (~10 s); clima gateado por `peak`/`community` del perfil F26; persianas escalonan arranques masivos. **v0.23.0: prioridad de cola por desviación + bypass de confort (REQ-PIC-5).** Presupuesto único de casa diferido. |
...
| **F06** | ✅ implementada | Sensor de energía (kWh) en VMC/DC/DS: medidor real o estimación; panel de Energía. Coste (€) en v0.24.0; **potencia instantánea** (W) por módulo + total de casa en v0.26.0 (REQ-ENE-5). Medidor real en DS diferido. |
| **F07** | ✅ implementada | Repairs transversal DV/DS/DC (mixin `DegradedTracker`): issue por módulo con fuentes requeridas ausentes/obsoletas >5min + evento `dynamic_home_degraded` + binary_sensor "Degradado". No-fixable + `learn_more_url`. **Botón fixable descartado** (v0.26.0): el issue ya se borra solo al recuperarse y un `ConfirmRepairFlow` no aporta sobre el enlace. |
...
| **F09** | ✅ implementada | Anti-ciclado opt-in: min ON/OFF + máx 6 arranques/h sobre el agregado del compresor compartido; vigila el ON/OFF que manda DC; la seguridad cede. Gating F26 (v0.17.0). **v0.23.0: hold por-emisor (solo bomba de calor).** **v0.27.0: canal de compresor completo** — `compressor_id` por emisor + `AntiCycleHub` multi-canal (dos bombas independientes no se interfieren; `"default"` = compresor único de casa). |
...
| **F25** | ✅ implementada | Multi-emisor por zona (1..N, `climate` y/o `switch`) con **staging primario/apoyo** (`staging.py`) y **conductos compartidos** (`shared_emitter.py` + `SharedEmitterHub`: agregación ponderada + guarda de undershoot + rejillas). Editor de emisores en opciones; un solo cerebro por zona; legacy single-device intacto. Canal de compresor por-emisor (`compressor_id`) en v0.27.0. |
| **F26** | ✅ implementada | Capa de declaración (v0.16.0): asistente 3D **generador×distribución×emisión**, defaults por inercia y perfil `community`/`compressor`/`peak`. **Gating F09/F03 cableado al perfil (v0.17.0); emisores F25 (v0.18.0).** **"Personalizado" (REQ-INS-7) descartado (v0.29.0): el catálogo cerrado cubre los casos reales y sostiene la validación por combinación.** |
...
| **F34** | 🟡 parcial | Módulo Energy (v0.21.0): singleton que publica `DATA_ENERGY` (headroom/tarifa/escasez; surplus gated por FV), no comanda; motor puro `energy_engine.py`; sensores + escasez; el headroom aprieta el presupuesto de F03. **§8.2 agregación de casa** (v0.24.0): `house_kwh`/`house_cost` (sensores Consumo/Coste de casa). **§8.3 sesgo de tarifa en DC** (v0.25.0, REQ-TAR-4): barato→más lead/preacondiciona, pico→menos lead. **FV (§8.5)/VE (§8.6) y balance neto §8.2 diferidos** (⚠️ FV no testable por el autor). |
...
| **F37** | ✅ implementada | Changeover comunitario (v0.20.0): dirección de casa heat/cool/off que las zonas `community` (F26) siguen; detección manual + auto por sensor de agua con umbrales; `changeover.py`; select + sensor en Zonas; `hvac_action`; opt-in/back-compat. **Refinado v0.22.0: histéresis de temporada (anti-flap) + override de changeover por zona (auto hereda la casa). Expuesto a DS (v0.28.0) y DV (v0.29.0).** Per-zona con sensor de agua propio descartado (v0.29.0). |
